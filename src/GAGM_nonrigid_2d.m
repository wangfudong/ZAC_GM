function para = GAGM_nonrigid_2d(X,Y,opt_reg,opt_gm)
...
while reg_cnt <= max_reg && reg_diff >= 1.0e-5 %%regist_err >= 1.0e-5 
    ...
    GAM_map = greedyMapping(GAM_soft, group1, group2);
    GAM_map = reshape(GAM_map, m, n);
    
    GAM_scores = reshape(GAM_soft, m, n).*GAM_map;
    [row,col] = find(GAM_map==1);
    max_scores = zeros(length(row),1);
    for ii = 1:length(row)
        max_scores(ii) = GAM_scores(row(ii),col(ii));
    end
