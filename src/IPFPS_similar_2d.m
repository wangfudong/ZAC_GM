function para = IPFPS_similar_2d(X,Y,opt_reg,opt_gm)
...
while reg_cnt <= max_reg && reg_diff >= 1.0e-4 %%regist_err >= 1.0e-5 
    ...
    IPFPS_map = greedyMapping(IPFPS_soft, group1, group2);
    IPFPS_map = reshape(IPFPS_map, m, n);
    
%     IPFPS_scores = reshape(IPFPS_soft, m, n).*IPFPS_map;
%     [row,col] = find(IPFPS_map==1);
%     max_scores = zeros(length(row),1);
%     for ii = 1:length(row)
%         max_scores(ii) = IPFPS_scores(row(ii),col(ii));
%     end
...
%     Map_all = zeros(size(IPFPS_map));
%     for j = 1:length(inds_max)
%         Map_all(row(inds_max(j)),col(inds_max(j)))=1;
%     end
  
    [R1,s1,t1] = rigid_parameter(X,Y,IPFPS_map);
