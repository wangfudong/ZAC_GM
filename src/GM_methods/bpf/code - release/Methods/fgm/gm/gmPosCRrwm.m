...
while bCont && iter_i < iterMax
    ...
    %    imagesc(reshape(prev_score, n1, n2));
        imagesc(reshape(prev_score, n2, n1));
   ...
   %     imagesc(reshape(cur_score, n1, n2));
        imagesc(reshape(cur_score, n2, n1));
    ...
    if isDisplay
     ...
     %   imagesc(reshape(cur_assign, n1, n2));
        imagesc(reshape(cur_assign, n2, n1));
 ...
 %   X = bistocNormalize_slack(reshape(cur_assign, n1, n2), tolC);
    X = bistocNormalize_slack(reshape(cur_assign, n2, n1), tolC);
    cur_assign = X(:);
...
X = reshape(cur_assign, size(E12, 2), size(E12, 1));
X = X';
end
