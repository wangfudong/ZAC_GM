...
nHist = ps(par, 'nHist', 10);
theta = ps(par, 'theta', 0.001);
sLen = 1;       % step length
x_store = zeros(sz1 * sz2, nHist);
...
while iAlp <= nAlp
    ...
    % FW
    [X,~,~,~,tm0] = mfw(X0, alp, nItMa, nHst, isDeb);
    tm = tm + tm0;
    ...
    if iAlp < nAlp && iAlp + sLen > nAlp  
        % force to optimize the last concave function to get discret solution
        iAlp = nAlp;
    else
        iAlp = iAlp + sLen;
    end
    ...
    x_store(:, nHist) = reshape(X, sz1 * sz2, 1);
...
%        X_est = reshape(Estimate(x_store), sz1, sz2);
        X_est = reshape(Estimate_scale(x_store, sLen_store), sz1, sz2);
        X = X_est;
% %         % normalize
% %         s = sum(X, 2) + eps;
% %         s = repmat(s, 1, sz2);
...
function x = Estimate3(x_store)
%% estimate next solution x according to previous K solutions
% x_store:  [N * K] matrix

x_vel = zeros(size(x_store));   % velocity
x_acc = zeros(size(x_store));   % acceleration

%% estimate acceleration and velocity of next solution
k = size(x_store, 2);
for i = 1:k-1
    x_vel(:,i) = x_store(:,i+1) - x_store(:,i);
end
for i = 1:k-2
    x_acc(:,i) = x_vel(:,i+1) - x_vel(:,i);
    x_acc(:,k-1) = x_acc(:,k-1) + i * x_acc(:,i);
end
x_acc(:,k-1) = x_acc(:,k-1) / sum(1:k-2);
x_vel(:,k) = x_vel(:,k-1) + x_acc(:,k-1);
x = x_store(:,k) + x_vel(:,k);
%x(x<0)=0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [X, objs, ts, nIt, tm] = mfw(X0, alp, nItMa, nHst, isDeb)
...
for nIt = 1 : nItMa
    ...
    tm(1) = tm(1) + cputime - tt;

    tt = cputime;
    % hungrian for computing the optimal direction
    Y = gmPosDHun(Gr);
    V = Y - X0;
    tm(2) = tm(2) + cputime - tt;
