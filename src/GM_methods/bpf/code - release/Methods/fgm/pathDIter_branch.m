function [path, X, X2, obj, nIts, Xs, objs, objGms, objCons, objVexs, objCavs, useIps, objInss, objIn2ss] ...
    = pathDIter_branch(X0, initAlp, alps, nItMa, nHst, isIp, isDeb, isDis, Y, isYM)
...
% path-following
prCIn('path', nAlp, .1);
path = zeros(nAlp - initAlp + 1, size(X0, 1), size(X0, 2));
