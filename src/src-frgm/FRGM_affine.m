...
while cnt <= max_regist && regist_tol_diff >= 1.0e-7%%regist_tol_diff >= 1.0e-2 
    if alter(cnt) == 0
    ...
    if cnt <= 5
        [V1,t1] = aff_sim_paremeters(X,Y,assign,SX,1,1,'affine2d');
    else
        [V1,t1] = aff_sim_paremeters(X,Y,asgHun(assign),SX,1,1,'affine2d');
    end
    ...
    Xtmp = X;
    X = rigid_affine_transform2D(X,V(:,:,cnt),t(:,:,cnt),1,[]);% updating X by geometry para
    ...
    regist_tol_diff = abs(regist_tol-regist_tol_old);
