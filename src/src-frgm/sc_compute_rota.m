function [BH,mean_dist]=sc_compute_rota(Bsamp,Tsamp,mean_dist,nbins_theta,nbins_r,r_inner,r_outer,out_vec)
...
%-----------------modified, for rotation invariant---------%
r_array=real(sqrt(dist2(Bsamp',Bsamp'))); % real is needed to
                                          % prevent bug in Unix version
center_B = Bsamp - repmat(mean(Bsamp,2),1,nsamp);
theta_center = repmat(atan2(center_B(2,:)',center_B(1,:)'),1,nsamp);

theta_array_abs = atan2(Bsamp(2,:)'*ones(1,nsamp)-ones(nsamp,1)*Bsamp(2,:),Bsamp(1,:)'*ones(1,nsamp)-ones(nsamp,1)*Bsamp(1,:))';
theta_array_abs = theta_array_abs - theta_center;
theta_array=theta_array_abs-Tsamp'*ones(1,nsamp);
