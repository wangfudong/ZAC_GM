function  [P, C, W, iter, T] = prgls_GRBF(X, Y, beta, lambda, max_it, tol, viz, outliers, corresp,sigma2, t, nsc,verbose)
...
hh = figure;subplot(1,2,1);
   set(hh,'position',[500 450 1000 300]);
    plot(Y(:,1),Y(:,2),'r.',X(:,1),X(:,2),'b.','markersize',20);
...
%     nbins_r,r_inner,r_outer,out_vec_1);
[BH1,mean_dist_1]=sc_compute_rota(X',zeros(1,nsamp1),mean_dist_global,nbins_theta,...
    nbins_r,r_inner,r_outer,out_vec_1);
...
%         nbins_r,r_inner,r_outer,out_vec_2);
      [BH2,mean_dist_2]=sc_compute_rota(T',zeros(1,nsamp2),mean_dist_1,nbins_theta,...
        nbins_r,r_inner,r_outer,out_vec_2);
...
%     outliers = 0.01;
    ...
    if viz
        subplot(1,2,2); 
        plot(T(:,1),T(:,2),'ro','linewidth',1.5,'markersize',6);
        hold on,plot(X(:,1),X(:,2),'b.','markersize',10);hold off;
        drawnow;
        %cpd_plot_iter(X, T); 
    end;
    iter=iter+1;
