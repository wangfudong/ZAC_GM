...
if ~isfield(opt,'verbose') || isempty(opt.verbose), opt.nsc = 1; end;
...
if opt.sparse
    [P, C, W, iter, T] = prgls_GRBF_sparse(X, Y, opt.beta, opt.lambda, opt.max_it, opt.tol, opt.viz, opt.outliers, opt.corresp, opt.sigma2, opt.t, opt.nsc,opt.verbose); 
else
    [P, C, W, iter, T] = prgls_GRBF(X, Y, opt.beta, opt.lambda, opt.max_it, opt.tol, opt.viz, opt.outliers, opt.corresp, opt.sigma2, opt.t, opt.nsc,opt.verbose); 
end
