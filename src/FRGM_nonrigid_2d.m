function para = FRGM_nonrigid_2d(X,Y,opt_reg,opt_frgm)
...
while reg_cnt <= max_reg && reg_diff >= 1.0e-5 %%regist_err >= 1.0e-5 
    ...
    Map_ini = asgHun(-M);
    sig_factor = 1;
    sig = sig_factor*[std(DXX(DXX(:)>0)).^2,std(DYY(DYY(:)>0)).^2];
    weight = [1,1];
    [asgFRGM] = FRGM_Gen(Map_ini,M,DXX,DYY,SX,opt_frgm,asgT,sig,weight);

    FRGM_scores = asgFRGM.map2;
    [row,col] = find(asgHun(asgFRGM.map2)==1);
