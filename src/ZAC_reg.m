function [Map,output] = ZAC_reg(X,Y,opt_gm,opt_reg)
...
%     SX = double_stochastic(SX,1000);
%     SY = double_stochastic(SY,1000);
% end
...
%     if length(opt_gm.index_priorX) >= 1
%         M(opt_gm.index_priorX,:) = 10;
%         if length(opt_gm.index_priorY) >= 1
%             M(:,opt_gm.index_priorY) = 10;
%         end
%     end

      Map_ini = kLAP_Hun(M,opt_gm.mass);
%     Map_ini = ones(LX,LY)/max(LX,LY);

    [Map,fw_output] = FW_minimu(Map_ini,M,SX,SY,A,B,opt_gm);
    ...
    if opt_gm.remove_time > 1 && cnt < opt_gm.remove_time
        ...
        if ~isfield(opt_gm,'out_re_type')
            ...
            [X_inl_ind,Y_inl_ind] = out_re(X_inl_ind,Y_inl_ind,Map,th,mass,opt_gm.out_re_type);
        end
