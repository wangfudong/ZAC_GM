function para = ZAC_similar_2d(X,Y,opt_reg,opt_gm)
...
file_write = 'save/rigid.gif';
if opt_reg.display > 0
    hh = figure('color','white');subplot('position',[0.02,0.05,0.45,0.9]);
    set(hh,'position',[500, 450 ,700 ,300]);
    plot(Y(yinl,1),Y(yinl,2),'b.',X(xinl,1),X(xinl,2),'r.','markersize',20);hold on;
    plot(Y(yout,1),Y(yout,2),'b+',X(xout,1),X(xout,2),'r+');
    hold off;
    axis off;
    ...
    if opt_reg.write > 0
        ...
        imwrite(imind,cm,file_write,'gif','WriteMode','overwrite', 'Loopcount',inf);
...
while reg_cnt <= 5 || (reg_cnt <= max_reg && reg_diff >= 1.0e-4) %%regist_err >= 1.0e-5 
    ...
    tm1 = clock;
    opt_reg.reg_cnt = reg_cnt;
    opt_reg.SC_or_DIS = unary_term(reg_cnt);
    [Map,output_gm] = ZAC_reg(X,Y,opt_gm,opt_reg);
    ...
    if opt_reg.display > 0
        disp(['ZAC: reg_err at iter ' num2str(reg_cnt) ': ' num2str(reg_err)]);
        disp(['ZAC: reg_diff at iter ' num2str(reg_cnt) ': ' num2str(reg_diff)]);
        disp(['ZAC: reg_time at iter ' num2str(reg_cnt) ': ' num2str(reg_time)]);
    end
    reg_cnt = reg_cnt + 1;
    ...
    if opt_reg.display > 0
        ...
        if opt_reg.write > 0
            ...
            im = frame2im(frame);
            [imind,cm] = rgb2ind(im,256);
            imwrite(imind,cm,file_write,'gif','WriteMode','append','DelayTime',0.4);
            % set(htext,'visible','off');
