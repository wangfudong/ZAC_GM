function para = MPM_nonrigid_2d(X,Y,opt_reg,opt_gm)
...
while reg_cnt <= max_reg && reg_diff >= 1.0e-5 %%regist_err >= 1.0e-5 
    ...
    asgMPM = MPM_GM( Pts{1,1}', Pts{1,2}',1:m,K,2500);
    
    MPM_scores = (asgMPM.X_con).*(asgMPM.map);
    [row,col] = find(asgMPM.map==1);
    max_scores = zeros(length(row),1);
    for ii = 1:length(row)
        max_scores(ii) = MPM_scores(row(ii),col(ii));
    end
