function para = PSM_nonrigid_2d(X,Y,opt_reg,opt_gm)
...
while reg_cnt <= max_reg && reg_diff >= 1.0e-5 %%regist_err >= 1.0e-5 
    ...
    PSM_map = greedyMapping(PSM_soft, group1, group2);
    PSM_map = reshape(PSM_map, m, n);
    
    PSM_scores = reshape(PSM_soft, m, n).*PSM_map;
    [row,col] = find(PSM_map==1);
    max_scores = zeros(length(row),1);
    for ii = 1:length(row)
        max_scores(ii) = PSM_scores(row(ii),col(ii));
    end
